...
class SearchBiomedicalLiteratureTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        if not arguments["query"]:
            yield self.create_text_message("A biomedical question or identifier is required.")
            return
        yield self.create_text_message(json.dumps(call_tool("search_biomedical_literature", arguments), ensure_ascii=False))
